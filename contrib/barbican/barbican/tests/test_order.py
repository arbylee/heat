...
class TestOrder(HeatTestCase):
    ...
    def setUp(self):
        ...
        self.res_template = self.template['Resources']['Order']
        self.res_template['Properties'] = self.props
        self._register_resources()

        self.patcher_client = mock.patch.object(order.client, 'Client')
    ...
    @mock.patch.object(order.client, 'barbican_client', new=mock.Mock())
    def test_handle_delete_ignores_not_found_errors(self):
        res = self._create_resource('foo', self.res_template, self.stack)

        order.client.barbican_client.HTTPClientError = Exception
        exc = order.client.barbican_client.HTTPClientError('Not Found. Nope.')
    ...
    def test_handle_delete_raises_resource_failure_on_error(self):
        ...
        order.client.barbican_client.HTTPClientError = Exception
        exc = order.client.barbican_client.HTTPClientError('Boom.')
        self.barbican.orders.delete.side_effect = exc
    ...
    def test_check_create_complete(self):
        ...
        mock_not_active = mock.Mock(status='NOT ACTIVE')
